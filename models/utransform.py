...
class TransformerLayer(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        x_flat = x.reshape(B, C, -1).permute(2, 0, 1)  # [HW, B, C]
        # 自注意力
        residual = x_flat
...
class UTransform(BaseModel):
    def __init__(self, num_classes, in_channels=5, freeze_bn=False, **_):
        ...
        self.transformer_layer = TransformerLayer(dim=1024)
        # U-Net 解码器部分
